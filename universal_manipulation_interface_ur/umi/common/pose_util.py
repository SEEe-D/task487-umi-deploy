...
def rot6d_to_mat(d6):
    ...
    # out = np.stack((b1, b2, b3), axis=-2) # 按行做stack
    out = np.stack((b1, b2, b3), axis=-1)  # 按列做stack
    return out

def mat_to_rot6d(mat):
    batch_dim = mat.shape[:-2]
    # out = mat[..., :2, :].copy().reshape(batch_dim + (6,)) # 取前两行
    out = np.swapaxes(mat[..., :, :2], -1, -2).copy().reshape(batch_dim + (6,))  # 取前两列
    return out
