...
class RTDEInterpolationController(mp.Process):
    ...
    def run(self):
        ...
        try:
            ...
            print("curr_pose before tx:", curr_pose)
            curr_pose = mat_to_pose(pose_to_mat(curr_pose) @ tx_flange_tip)
            print("curr_pose after tx:", curr_pose)
            ...
            while keep_running:
                ...
                if not in_teach_mode:
                    pose_ = pose_interp(t_now)
                    # print("pose_:", pose_)
                    pose_command = mat_to_pose(pose_to_mat(pose_) @ tx_tip_flange)
                    vel = 0.5
                    acc = 0.5
                ...
                flange_pose = state["ActualTCPPose"]
                state["ActualTCPPose"] = mat_to_pose(pose_to_mat(flange_pose) @ tx_flange_tip)
