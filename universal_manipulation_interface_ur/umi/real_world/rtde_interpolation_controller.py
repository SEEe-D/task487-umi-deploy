...
# from umi.common.cubic_pose_trajectory_interpolator import CubicPoseTrajectoryInterpolator
# from umi.common.cubic_linear_pose_trajectory_interpolator \
...
class RTDEInterpolationController(mp.Process):
    ...
    def __init__(self,
            ...
            force_limit=20.0,
            force_backoff_step=0.015,
            torque_backoff_step=0.015,
            launch_timeout=3,
        ...
        assert 0 < force_limit
        assert 0 < force_backoff_step
        assert 0 < torque_backoff_step
        ...
        self.force_backoff_step = force_backoff_step
        self.torque_backoff_step = torque_backoff_step
    ...
    def run(self):
        ...
        try:
            ...
            while keep_running:
                ...
                # if self.robot_ip == "192.168.3.244":
                #     print(self.robot_ip, "RTDE:", state["ActualTCPForce"])
                
                actual_tcp_force = state["ActualTCPForce"]
                force_over_limit = np.abs(actual_tcp_force) > self.force_limit

                # print(self.robot_ip, "RTDE:", state["ActualTCPPose"])
                ...
                for i in range(n_cmd):
                    ...
                    #     # 先停掉 servo，和测试脚本一样直接调用 teachMode
                    #     in_teach_mode = True
                    #     rtde_c.servoStop()
                    #     time.sleep(0.3)
                    #     ret = rtde_c.teachMode()
                    #     print(f"[RTDEPositionalController] teachMode() returned: {ret}")
                    # elif cmd == Command.END_TEACH_MODE.value:
                    #     ret = rtde_c.endTeachMode()
                    #     print(f"[RTDEPositionalController] endTeachMode() returned: {ret}")
                    #     in_teach_mode = False
                    #     # 重置位姿插值器到当前实际位姿
                    #     curr_pose = rtde_r.getActualTCPPose()
                    #     curr_t = time.monotonic()
                ...
                # if (not in_teach_mode) and np.any(force_over_limit):
                #     # Back off in each exceeded wrench dimension.
                #     # Translation (xyz) and rotation (rx, ry, rz) use different retreat step sizes.
                #     retreat_delta = np.zeros(6, dtype=np.float64)
                #     retreat_delta[:3] = (
                #         -np.sign(actual_tcp_force[:3]) * force_over_limit[:3] * self.force_backoff_step
                #     )
                #     retreat_delta[3:] = (
                #         -np.sign(actual_tcp_force[3:]) * force_over_limit[3:] * self.torque_backoff_step
                #     )

                #     curr_time = t_now + dt
                #     retreat_pose = pose_interp(curr_time) + retreat_delta
                #     retreat_duration = max(dt, 0.06)
                #     t_insert = curr_time + retreat_duration
                #     pose_interp = pose_interp.drive_to_waypoint(
                #         pose=retreat_pose,
                #         time=t_insert,
                #         curr_time=curr_time,
                ...
                #     if (time.monotonic() - last_collision_print_t) > 0.5:
                #         print(
                #             f"[RTDEPositionalController] Collision backoff on {self.robot_ip}, "
                #             f"wrench={actual_tcp_force}, retreat={retreat_delta}"
                #         )
                #         last_collision_print_t = time.monotonic()
