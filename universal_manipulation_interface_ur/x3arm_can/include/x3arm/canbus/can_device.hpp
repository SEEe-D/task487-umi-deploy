...
public:
    ...
    // Get all receive CAN IDs for this device (for multi-CAN ID support)
    // Default implementation returns single recv_can_id_
    // Subclasses can override to return multiple CAN IDs
    virtual std::vector<canid_t> get_all_recv_can_ids() const {
        return {recv_can_id_};
    }
