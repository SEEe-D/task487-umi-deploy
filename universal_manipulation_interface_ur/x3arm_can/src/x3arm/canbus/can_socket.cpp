...
CANSocket::CANSocket(const std::string &interface, bool enable_fd)
  ...
  if (!initialize_socket(interface)) {
    throw CANSocketException("Failed to initialize socket for interface: " +
                             interface);
  }
}
...
bool CANSocket::write_can_frame(const can_frame &frame) {
  if (traffic_callback_)
    traffic_callback_(frame, true);
  return write(socket_fd_, &frame, sizeof(frame)) == sizeof(frame);
}

bool CANSocket::write_canfd_frame(const canfd_frame &frame) {
  if (traffic_callback_ && frame.len <= 8) {
    can_frame legacy_frame;
    legacy_frame.can_id = frame.can_id;
    legacy_frame.can_dlc = frame.len;
    memcpy(legacy_frame.data, frame.data, frame.len);
    traffic_callback_(legacy_frame, true);
  }
  return write(socket_fd_, &frame, sizeof(frame)) == sizeof(frame);
}
...
bool CANSocket::read_canfd_frame(canfd_frame &frame) {
  ...
  if (bytes_read == sizeof(frame)) {
    if (traffic_callback_ && frame.len <= 8) {
        ...
        legacy_frame.can_dlc = frame.len;
        memcpy(legacy_frame.data, frame.data, frame.len);
        traffic_callback_(legacy_frame, false);
    }
    return true;
  }
  return false;
