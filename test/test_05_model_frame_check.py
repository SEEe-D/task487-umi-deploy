...
from universal_manipulation_interface_ur.umi.common.pose_util import (
...
def robot_pose_to_model_pose(robot_pose, pose10d=True):
    model_pose = robot_pose.copy()
    model_pose[:3] = R_R2M @ robot_pose[:3]
    robot_rot_mat = st_Rotation.from_rotvec(robot_pose[3:6]).as_matrix()
    model_pose[3:6] = st_Rotation.from_matrix(R_R2M @ robot_rot_mat).as_rotvec()
    if pose10d:
        model_pose = mat_to_pose10d(pose_to_mat(model_pose))
    
    return model_pose

while True:
    tcp = rtde_r.getActualTCPPose()
    model_pose = robot_pose_to_model_pose(tcp, pose10d=False)
    print(f"TCP(世界): x={tcp[0]:+.4f} y={tcp[1]:+.4f} z={tcp[2]:+.4f}\n", end="")
    print(f"Model   : x={model_pose[0]:+.4f} y={model_pose[1]:+.4f} z={model_pose[2]:+.4f}", end="")
    cmd = input("  > ").strip()
