...
CONFIG_NAME="pi05_block_btmcam_h20"
CKPT_DIR="/workspace/zt/checkpoints/${CONFIG_NAME}/${CONFIG_NAME}"
STATS_DIR="assets/${CONFIG_NAME}/local/block_btmcam"

# 1. 数据转换
echo ">>> 转换数据: blockbtmcam (下方相机)"
python examples/umi/convert_zarr_to_lerobot.py \
    --zarr_path /workspace/umi/mydata/blockbtmcam.zarr.zip \
