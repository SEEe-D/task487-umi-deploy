...
class RMSNorm(nn.Module):
    ...
    def __call__(self, x, cond):
        ...
        # Standard Pi0.5 uses one condition per batch item [B,D]. RTC uses
        # t=0 for fixed-prefix tokens and the current flow time for generated
        # suffix tokens, producing a per-token condition [B,T,D].
        if modulation.ndim == x.ndim - 1:
            modulation = modulation[:, None, :]
        elif modulation.ndim != x.ndim:
            raise ValueError(
                f"AdaRMS condition rank {modulation.ndim} is incompatible with input rank {x.ndim}"
            )
