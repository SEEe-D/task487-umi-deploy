...
class MaitaCANDevice : public canbus::CANDevice {
public:
    explicit MaitaCANDevice(Motor& motor, canid_t recv_can_mask, bool use_fd);
    void callback(const can_frame& frame) override;
    void callback(const canfd_frame& frame) override;

    // Override to return all receive CAN IDs (MIT and Query)
