...
// Interactive Demo - Single Motor Test (MTRH32) with Protocol Documentation
//
// ============================================================================
...
void print_state(const Motor& m) {
    std::cout << "id=" << m.get_device_id() << " (" << motor_type_to_string(m.get_motor_type()) << ") "
              << std::fixed << std::setprecision(2) << "pos=" << std::setw(8)
              << m.get_position() * 180.0 / M_PI << " deg"
              << ", vel=" << std::setw(7) << m.get_velocity() * 180.0 / M_PI << " dps"
...
void print_mit_state(const Motor& m) {
    std::cout << "id=" << m.get_device_id() << " (" << motor_type_to_string(m.get_motor_type()) << ") "
              ...
              << m.get_position() * 180.0 / M_PI << " deg"
              << ", vel=" << std::setw(7) << m.get_velocity() * 180.0 / M_PI << " dps"
              << ", torque=" << std::setw(6) << m.get_torque() << " Nm" << std::endl;
}
...
int main(int argc, char* argv[]) {
    ...
    try {
        std::string can_if = argc > 1 ? argv[1] : "can4";
        std::cout << "Init on " << can_if << "..." << std::endl;

        x3arm::can::socket::X3Arm arm(can_if, false);

        // 单电机：MTRH32 @ device_id=0x01（如需修改 ID，请改这里）
        arm.init_arm_motors({MotorType::MTRH32}, {0x03});

        auto motors = arm.get_arm().get_motors();
        std::cout << "Motors on bus: " << motors.size() << std::endl;
        for (size_t i = 0; i < motors.size(); ++i) {
            const auto& m = motors[i];
            std::cout << "  [" << i << "] " << motor_type_to_string(m.get_motor_type())
                      << " @ device_id=" << hex_u32(m.get_device_id()) << std::endl;
            std::cout << "       Send (Query): " << hex_u32(m.get_query_send_can_id())
                      << ", (MIT): " << hex_u32(m.get_mit_send_can_id()) << std::endl;
            std::cout << "       Recv (Query): " << hex_u32(m.get_query_recv_can_id())
                      << ", (MIT): " << hex_u32(m.get_mit_recv_can_id()) << std::endl;
        }
        std::cout << std::endl;

        int motor_idx = 0;  // 当前选择的电机 index（单电机时恒为 0）
        std::string input;
        while (g_running) {
            ...
            std::cout << "\n=== Single Motor Test (MTRH32) ===" << std::endl;
            std::cout << "Current: [" << motor_idx << "] " << motor_type_to_string(cur_m.get_motor_type())
            ...
            if (input == "s" || input == "S") {
                ...
                for (size_t i = 0; i < motors.size(); ++i) {
                    std::cout << "[RECV] "
                              << hex_u32(arm.get_arm().get_motor(static_cast<int>(i)).get_query_recv_can_id())
                              << ": ";
                ...
                std::cout << "[SEND] " << hex_u32(cur_m.get_query_send_can_id())
                          << ": 0xA4 (PosClosedLoop), target=" << target << " (0.01deg)" << std::endl;
                ...
                std::cout << "[SEND] " << hex_u32(cur_m.get_query_send_can_id())
                          << ": 0xA2 (SpeedClosedLoop), speed=" << speed << " (0.01dps)" << std::endl;
                arm.set_callback_mode_all(CallbackMode::IGNORE);
                ...
                std::cout << "Target (deg): ";
                std::getline(std::cin, input);
                double target_rad = std::stod(input) * M_PI / 180.0;

                MITParam params = {kp, kd, target_rad, 0.0, 0.0};
                arm.set_callback_mode_all(CallbackMode::STATE);

                std::cout << "[SEND] " << hex_u32(cur_m.get_mit_send_can_id())
                          << ": MIT mode, kp=" << kp << ", kd=" << kd << ", target=" << target_rad
                          << " rad" << std::endl;
                std::cout << "MIT running 3s (requires continuous sending)..." << std::endl;
                for (int i = 0; i < 300 && g_running; ++i) {
                    arm.get_arm().mit_control_one(motor_idx, params);
                    arm.recv_all(1000);  // 1ms timeout
                    if (i % 10 == 0) {
                        std::cout << "[RECV] " << hex_u32(cur_m.get_mit_recv_can_id()) << " [" << i * 10
                                  << "ms] ";
                        print_mit_state(arm.get_arm().get_motor(motor_idx));  // MIT 不含 temp
                    }
                ...
                std::cout << "NOTE: Requires 0x76 system reset to take effect!" << std::endl;
                ...
                std::cout << "NOTE: Requires 0x76 system reset to take effect!" << std::endl;
                std::cout << "WARNING: This writes zero offset to ROM for ALL motors." << std::endl;
                std::cout << "Confirm? (y/n): ";
                std::getline(std::cin, input);
                if (input == "y" || input == "Y") {
                    arm.set_callback_mode_all(CallbackMode::IGNORE);
                    ...
                    arm.recv_all(1000);
                    std::cout << "[RECV] OK: Zero offset written to ROM (all)." << std::endl;
                } else {
                    std::cout << "Cancelled." << std::endl;
                }
                ...
                arm.recv_all(1000);
                std::cout << "[RECV] " << hex_u32(cur_m.get_query_recv_can_id()) << ": Value = "
                          << arm.get_arm().get_motor(motor_idx).get_param(static_cast<ReadParamIndex>(idx))
                          << std::endl;
                ...
                std::cout << "[SEND] " << hex_u32(cur_m.get_mit_send_can_id())
                          ...
                          << "kp=" << kp << ", kd=" << kd << ", t_ff=" << torque_ff << " Nm"
                          << std::endl;

                arm.get_arm().mit_control_one(motor_idx, params);
                arm.recv_all(1000);  // Wait 1ms for response
                std::cout << "[RECV] " << hex_u32(cur_m.get_mit_recv_can_id()) << ": ";
                print_mit_state(arm.get_arm().get_motor(motor_idx));  // MIT 响应不含 temp

                std::cout << "\nNow STOP MIT, query via 0x9C for 5s..." << std::endl;
                std::cout << "(If motor stops, MIT needs continuous sending)" << std::endl;
                for (int i = 0; i < 100 && g_running; ++i) {
                    arm.read_state_one(motor_idx);  // Query via 0x9C
                    arm.recv_all(500);
                    std::cout << "[RECV] " << hex_u32(cur_m.get_query_recv_can_id()) << " ["
                              << (i + 1) * 100 << "ms] ";
                    print_state(arm.get_arm().get_motor(motor_idx));  // State2 响应包含 temp
                    std::this_thread::sleep_for(std::chrono::milliseconds(100));
                }
                ...
                if (input == "y" || input == "Y") {
                    ...
                    std::cout << "Reset sent. Motor restarting (no reply)..." << std::endl;
                ...
                if (input == "y" || input == "Y") {
                    ...
                    std::cout << "Reset sent. Motors restarting (no reply)..." << std::endl;
                    std::this_thread::sleep_for(std::chrono::milliseconds(2000));
                    std::cout << "Motors should be ready now. (You can query via 0x9C)" << std::endl;
                } else {
                    std::cout << "Cancelled." << std::endl;
                }
