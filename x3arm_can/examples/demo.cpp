...
int main(int argc, char* argv[]) {
    ...
    try {
        std::string can_if = argc > 1 ? argv[1] : "can2";
        std::cout << "Init on " << can_if << "..." << std::endl;

        x3arm::can::socket::X3Arm arm(can_if, false);
        // 两电机：index=0 -> MTX660@id=0x01, index=1 -> MTX436@id=0x02
        arm.init_arm_motors({MotorType::MTX660, MotorType::MTX660, MotorType::MTX436,
            MotorType::MTX436, MotorType::MTX436, MotorType::MTX436, MotorType::MTX436, MotorType::MTX410},
            {0x01, 0x02, 0x03, 0x04, 0x05, 0x06, 0x07, 0x08});
        // arm.init_arm_motors({MotorType::MTX660}, {0x01});
        ...
        while (g_running) {
            ...
            if (input == "s" || input == "S") {
                ...
                arm.recv_all(1000);
                for (size_t i = 0; i < motors.size(); ++i) {
                    std::cout << "[RECV] " << hex_u32(arm.get_arm().get_motor(static_cast<int>(i)).get_query_recv_can_id())
                              << ": ";
